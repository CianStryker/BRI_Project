...
plot10 <- ggplot(FDI_GDP, aes(x = log_ycum,  y = log_GDP, color  = "red")) +
   geom_jitter() +
...
linearMod <- lm(FDI ~ GDP, data=FDI_GDP)  
plot11 <- summ(linearMod)
```
...
```{r}
plot12 <- ggplot(test4, aes(x = FDI, size = FDI, color = region, y = GDP_Increase)) +
  geom_point() +
...
plot13 <- ggplot(test5, aes(x = FDI, size = FDI, color = region, y = GDP_Increase)) +
   geom_point() +
   geom_smooth(method = "lm", se = FALSE) + 
