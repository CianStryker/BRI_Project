...
ggplot(test49, aes(x = log_ycum,  y = log_GDP, color  = "red")) +
   geom_jitter() +
...
linearMod <- lm(FDI ~ GDP_Increase, data=test48)  
summ(linearMod)
```
...
ggplot(test5, aes(x = FDI, size = FDI, color = region, y = GDP_Increase)) +
   geom_point() +
   geom_smooth(method = "lm", se = FALSE) + 
  geom_text(data = subset(test5, FDI > 2e+10), aes(label = country2, hjust = 1.3, vjust = 1)) +
  labs(x= "Cummalitve BRI Investment (in Billions USD)",
       y= "GDP PPP (in Billions USD)",
       fill=NULL,
