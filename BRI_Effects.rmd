---
title: "BRI_Effects"
author: "Cian Stryker"
date: "10/17/2019"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)

library(readxl)
library(reshape2)
library(plyr)
library(jtools)
library(kableExtra)
library(countrycode)
library(janitor)
library(gvlma)
library(maps)
library(png)
library(cowplot)
library(reprex)
library(ggrepel)
library(gganimate)
library(tidyverse)
```

```{r, Chinese FDI Loading, message=FALSE}
Chinese_FDI <- read_excel("raw-data/Chinese_Investment_and_Construction.xlsx", skip = 4) %>%
  clean_names()

Chinese_FDI$country <- countrycode(Chinese_FDI$country, "country.name", "country.name")
Chinese_FDI$country2 = Chinese_FDI$country

Chinese_FDI$country2 <- countrycode(Chinese_FDI$country2, "country.name", "iso2c")
```

```{r, GDP loading, message=FALSE, warning=FALSE}
GDP_An <- read_excel("raw-data/GDP.xls", skip = 2) 
 
GDP2 <- melt(GDP_An, measure.vars = 2:15) %>%
  clean_names()

colnames(GDP2)[colnames(GDP2) == 'variable'] <- 'year'
colnames(GDP2)[colnames(GDP2) == 'value'] <- 'GDP_Increase'
colnames(GDP2)[colnames(GDP2) == 'country_name'] <- 'country'

GDP2$country <- countrycode(GDP2$country, "country.name", "country.name") 
GDP2$country2 = GDP2$country

GDP2$country2 <- countrycode(GDP2$country2, "country.name", "iso2c")

GDP_Final <- na.omit(GDP2)
```

```{r, Mapwold loading, warning=FALSE}

map.world <- map_data('world')
colnames(map.world)[colnames(map.world) == 'region'] <- 'country'
map.world$country <- countrycode(map.world$country, "country.name", "country.name")

```

```{r}
test99 <- Chinese_FDI %>%
  select("country2", "region", "quantity_in_millions", "year") 

test29 <- aggregate(quantity_in_millions ~ country2 + region + year, data = test99, FUN = sum)

test39 <- test29 %>%
  complete(year, nesting(country2, region), fill = list(quantity_in_millions=0)) %>%
  arrange(year) %>%
  group_by(country2, region) %>%
  mutate(y_cum4 = cumsum(quantity_in_millions)) %>%
  mutate(FDI = y_cum4 * 1000000) 

test48 <- merge(test39, GDP_Final, by = c("year", "country2")) %>%
  mutate(log_ycum = log10(FDI)) %>%
  mutate(log_GDP = log10(GDP_Increase))

test49 <- merge(test39, GDP_Final, by = c("year", "country2")) %>%
  filter(year == 2018) %>%
  filter(GDP_Increase <= 1.0e+13 ) %>%
  mutate(log_ycum = log10(FDI)) %>%
  filter(log_ycum >= 0) %>%
  mutate(log_GDP = log10(GDP_Increase)) 

maybe <- test49 %>%
  summarize(coorelation = cor(y_cum4, GDP_Increase))

ggplot(test49, aes(x = log_GDP,  y = log_ycum, color  = "red")) +
   geom_jitter() +
  labs(x= "GDP PPP (in Billions USD)",
       y= "BRI Chinese FDI (in Billions USD)",
       fill=NULL,
       title= "Regression of All Chinese FDI Recipient Countries") +
  guides(color = FALSE) +
   geom_smooth(method = "lm", se = FALSE) 
```

```{r render = 'normal_print'}
linearMod <- lm(GDP_Increase ~ FDI, data=test48)  
summ(linearMod)
```

```{r, BRI and GDP Scatter Plot }
test <- Chinese_FDI %>%
  select("country2", "quantity_in_millions", "year","sector", "region") %>%
  filter(region %in% c("Arab Middle East and North Africa", "East Asia", "South America", "Sub-Saharan Africa", "West Asia")) %>%
  filter(sector %in% c("Energy", "Real estate", "Metals", "Transport")) 

test2 <- aggregate(quantity_in_millions ~ country2 + year + region, data = test, FUN = sum)

test3 <- test2 %>%
  complete(year, nesting(country2, region), fill = list(quantity_in_millions=0)) %>%
  arrange(year) %>%
  group_by(region, country2) %>%
  mutate(y_cum4 = cumsum(quantity_in_millions)) %>%
  mutate(FDI = y_cum4 * 1000000)

test4 <- merge(test3, GDP_Final, by = c("year", "country2")) 
```

```{r}
ggplot(test4, aes(x = FDI, size = FDI, color = region, y = GDP_Increase)) +
  geom_point() +
  geom_text(data = subset(test4, FDI > 3e+10 | GDP_Increase > 1e+12), aes(label = country2, hjust = 0, vjust = -1.3)) +
  labs(x= "BRI Chinese FDI (in Billions USD)",
       y= "GDP PPP (in Billions USD)",
       fill=NULL,
       title="Year {closest_state}") +
  transition_states(year, transition_length = 4, state_length = 4) +
  enter_fade() +
  guides(size = FALSE, color = FALSE) +
  ease_aes('sine-in-out')+
  facet_wrap(~region)
```

```{r}
test5 <- test4 %>%
  filter(year == 2018)

ggplot(test5, aes(x = FDI, size = FDI, color = region, y = GDP_Increase)) +
   geom_point() +
   geom_smooth(method = "lm", se = FALSE) + 
  geom_label_repel(data = subset(test5, FDI > 2.5e+10), aes(label = country2, hjust = 1.3, vjust = 1)) +
  labs(x= "Cummulative BRI Investment (in Billions USD)",
       y= "GDP PPP (in Billions USD)",
       fill=NULL,
       title= "Commulative FDI with GDP in 2019") +
  guides(size = FALSE, color = FALSE) +
  facet_wrap(~region)
```



