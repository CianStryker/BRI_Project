...
```{r, warning=FALSE}
Debt <- read_excel("raw-data/imf-debt.xls")
Debt2 <- melt(Debt, measure.vars = 2:16) %>%
  clean_names() 

Debt2$value <- as.numeric(as.character(Debt2$value))

colnames(Debt2)[colnames(Debt2) == 'variable'] <- 'year'
colnames(Debt2)[colnames(Debt2) == 'value'] <- 'debt'
colnames(Debt2)[colnames(Debt2) == 'general_government_gross_debt_percent_of_gdp'] <- 'country'

Debt2$country <- countrycode(Debt2$country, "country.name", "country.name") 
Debt2$country2 = Debt2$country

Debt2$country2 <- countrycode(Debt2$country2, "country.name", "iso2c")
...
ggplot(China_1y, aes(x = year, y = y_cum, group = region)) +
  geom_line(aes(linetype= "solid", color= region)) +
...
China_sector <- Chinese_FDI %>%
    ...
    filter(region %in% c("Arab Middle East and North Africa", "East Asia", "South America", "Europe", "North America", "Sub-Saharan Africa", "US", "West Asia")) 
