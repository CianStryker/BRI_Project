...
```{r}
ggplot(x5, aes(x = PR, y = FDI, fill = PR)) + 
  geom_col() +
...
```{r}
cowplot <- plot_grid(plot_1, plot_2, labels = NULL)
```

```{r}
x9 <- merge(x6, GDP_Final) %>%
  mutate(log_y = log10(y_cum5), 
         log_GDP = log10(GDP_Increase)) 

x10 <- x9 %>%
  filter(PR %in% c("1", "2", "3", "4", "5", "6", "7")) %>%
  filter(log_y >= 0)

ggplot(x10, aes(x = log_y,  y = log_GDP, color = PR)) +
  geom_jitter() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(x= "Log of Cummulative BRI Investment in Billions",
       y= "GDP in Billions USD",
       fill=NULL,
       title= "BRI Investment and GDP per Level of Political Rights in 2019") +
  facet_wrap(~PR)
```

```{r}
linearMod3 <- lm(y_cum5 ~ GDP_Increase + PR, data=x9)  
print(linearMod3)
summ(linearMod3)
```
